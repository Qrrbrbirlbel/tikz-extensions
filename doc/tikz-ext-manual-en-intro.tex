...
\begin{key}{/\tikzext/compat=\mchoice{pre 0.6, 0.6, warn, newest} (default pre 0.6)}
  ...
  \begin{tabular}{>{\ttfamily ext.}l ccc}
    ...
    \multicolumn{1}{l}{\ttfamily pgfcalendar-ext}        & \X   &   \X    & \Xdef/-- \\
    \multicolumn{1}{l}{\quad\ttfamily ext.calendar-plus} &      &         &          \\ \midrule[.5\lightrulewidth]
    arrows                                               & \X   &   \X    & \Xdef/-- \\
    layers                                               & \X   &   \X    & \Xdef/-- \\
    node-families                                        & \X   &   \X    & \Xdef/-- \\
    nodes                                                & \X   &   \X    & \Xdef/-- \\
    paths.arcto                                          & \X   &   \X    & \Xdef/-- \\ \addlinespace
    paths.ortho                                          & \X   &   \X    & \Xdef/-- \\
    paths.timer                                          & \X   &   \X    & \Xdef/-- \\
    patterns.images                                      & \X   &   \X    & \Xdef/-- \\
    pgffor-ext                                           & \X   &   \X    & \Xdef/-- \\
    pgfkeys-plus                                         & \X   &   \X    & \Xdef/-- \\
    positioning-plus                                     & \X   &   \X    & \Xdef/-- \\ \addlinespace
    scalepicture                                         & \X   &   \X    & \Xdef/-- \\
    shapes                                               & \X   &   \X    & \Xdef/-- \\
    transformations.mirror                               & \X   &   \X    & \Xdef/-- \\
    topaths.arcthrough                                   & \X   &   \X    & \Xdef/-- \\ \bottomrule
  \end{tabular}
  \end{center}
