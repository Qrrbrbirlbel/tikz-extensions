...
\begin{key}{/\tikzextnp/add anchor osep=\marg{n}\marg{c}\marg{ox}\marg{oy}}
...
\end{stylekey}
\begin{codeexample}[preamble=\usetikzlibrary{decorations.pathreplacing, ext.nodes},width=3.75cm]
\begin{tikzpicture}[
  ext/nodes/install auto offset for brace decoration,
  decoration={
    name=brace, aspect=.3, raise=2mm, amplitude=4mm},
]
\draw[help lines] (0,0) -- (2,1)
        (-.5,-.5) grid (2.5,1.5);
\draw[decorate] (0,0) --
  node[draw, ext/every brace node] {Foo} (2,1);
\end{tikzpicture}
\end{codeexample}
\end{key}
...
As the following example shows, this might be most useful for circular nodes.
\begin{codeexample}[preamble=\usetikzlibrary{ext.nodes}]
\begin{tikzpicture}[
  auto=ext/right, nodes={draw=blue, pos=\i/10}, outer sep=+0pt,
  text depth=+0pt, out=-15, in=15, line width=1mm
] \newcommand*\List{1, 3, 4, 5, 6, 7, 9}
\draw (right:0) to node foreach \i in \List                         {\i} +(up:3);
\draw (right:2) to node foreach \i in \List[ext/precise auto angle] {\i} +(up:3);
\tikzset{shift=(right:4), shape=circle}
\draw (right:0) to node foreach \i in \List                         {\i} +(up:3);
\draw (right:2) to node foreach \i in \List[ext/precise auto angle] {\i} +(up:3);
\end{tikzpicture}
\end{codeexample}
...
This library redefines the keys \referenceKeyandIndexO{auto},
...
\begin{key}{/\tikzext/nodes/install auto=\marg{left}\marg{right}}
  This key defines
  \begin{itemize}
  \item \texttt{/tikz/auto/\meta{left}} and
  \item \texttt{/tikz/auto/\meta{left}@precise}
        which both activate the auto placement for \meta{left}
        as well as
  \item \texttt{/tikz/swap/\meta{left}} and
  \item \texttt{/tikz/swap/\meta{left}@precise}
        which both swap to the \meta{right} placement function.
  \end{itemize}
  
  This already shows that the precise versions are just separate placement functions.
