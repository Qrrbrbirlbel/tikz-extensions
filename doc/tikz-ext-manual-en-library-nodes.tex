...
\begin{description}
\item[nest] whether the feature can be nested (matrices cannot be nested)
\item[ref out] whether one can reference coordinates/nodes outside of a feature (you can't reference a node in the outer \tikzname picture from inside a matrix)
\item[ref in] whether one can reference coordinates/nodes from outside of a feature (nested \tikzname pictures need |remember picture|)
\item[trans] whether the feature can be transformed (a \pgfname\ matrix can only be shifted)
\item[inherit] whether settings from outside the feature gets inherited -- this is usually something we have to be careful about, only the \pgfname matrix allows some protection against it
\item[placing] whether a feature can be placed well (a \tikzname\ pic doesn't provide any anchors)
\end{description}
The introduced Node Picture shall offer a compromise between a \tikzname\ pic and a \pgfname\ matrix
...
\item The baseline of the node is the picture's $y$ value of $0$, it can be changed, though, by using |/tikz/ext/baseline| (see below).
...
\end{predefinednode}
\begin{predefinednode}{current node picture bounding box}
