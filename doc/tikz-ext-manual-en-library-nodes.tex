...
\item[ref out] whether one can reference coordinates/nodes outside of a feature
  ...
  You can't reference a node in the outer \tikzname picture from inside a matrix.
  Referencing node from the outer scope while inside a Node Picture can be done
  if using the |absolute| key (see below).
  You probably want to use |overlay| so that the path that connect to an outer node
  does not change the Node Picture -- or maybe that's exactly what you want \dots
...
\item The baseline of this text is the vertical center of the Node Picture's bounding box.
      ...
      This must be set \emph{inside} the Node Picture!
\begin{codeexample}[preamble=\usetikzlibrary{ext.nodes}]
\begin{tikzpicture}
\foreach[count=\i] \b in {north, base, south}
  \node[
    ext/node picture, anchor=base,
    draw, fill=blue!50
  ] at (right:2*\i) {
    \tikzextset{node picture/baseline=(n.\b)}
    \node[fill=red!50, draw] (n) {\b $\displaystyle \frac12$};
  };
\draw[thick, green] (1,0) -- (right:7);
\end{tikzpicture}
\end{codeexample}
...
\item Just like with a pic or a matrix, certain settings and values will be inherited by the Node Picture. (It's a gift and a curse.)
...
This key will be applied for every node picture,
this will be executed on the same as \referenceKeyandIndexO{every node}.
...
\end{key}
\begin{key}{/\tikzext/node picture/prefix name=\opt{\meta{true or false}} (default true, initially true)}
...
This must be used \emph{inside} a Node Picture to have an effect on that Node Picture.
\end{key}
...
\draw[green] (1,0) -- (right:7);
\end{tikzpicture}
\end{codeexample}
\end{key}
\begin{key}{/\tikzext/node picture/absolute=\opt{\meta{coordinate}} (default 0,0)}
This key syncs the Node Picture's coordinate system with the outer scope's coordinate system.
This sets both |ext/node picture/anchor| and |at| to the same value.

This also may make it possible to sucessfully reference nodes outside of a Node Picture.
You probably want to use |overlay| on any connection, though.
\begin{codeexample}[preamble=\usetikzlibrary{ext.nodes},width=2.7cm]
\begin{tikzpicture}[nodes=draw]
\node[fill=red] at (2,1) (ABC) {ABC};
\node[ext/node picture={true, absolute}, fill=green]{
  \node[fill=blue!50] (DEF) at (1,0) {DEF} edge[overlay] (ABC);
