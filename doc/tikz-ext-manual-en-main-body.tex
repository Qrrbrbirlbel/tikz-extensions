...
%  tikz-ext-manual-en-pgf-trans,
  tikz-ext-manual-en-pgf-shapes-any,
...
\include{tikz-ext-manual-en-pgf-arrows}
\include{tikz-ext-manual-en-pgf-trans}
\include{tikz-ext-manual-en-pgf-shapes-any}
...
\include{tikz-ext-manual-en-library-pgffor}
...
\newcommand*{\addShape}[2][]{Added shape \texttt{ext\textunderscore\ifx\\#1\\#2\else#1\fi}\\(\pgfname\space library \texttt{ext.shapes.#2}).}
...
\begin{itemize}
\item Bugfix to |ext.positioning-plus|. \cite{GH24}
\item Bugfix to |ext.beamer|.
\item Bugfix to |ext.paths.ortho|.
\item Added node pictures to |ext.nodes|.
\item Added |auto| key to |ext.topaths.arcthrough|.
\item |warn| is not the default compatibility setting anymore.
\end{itemize}
