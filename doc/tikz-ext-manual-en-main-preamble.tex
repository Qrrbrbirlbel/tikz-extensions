...
\usetikzlibrary{
  ...
  graphs,
  matrix,
  offsetpath, % (part of nfold) for ext_any
  shapes.geometric,
  spath3,
  through,
  trees,
...
\newcommand*\tikzextnp{tikz/ext/node picture}
...
\newenvironment{ext_predefinednode}[1]{
  \begin{pgfmanualentry}
    \pgfmanualentryheadline{%
      \pgfmanualpdflabel{ext_#1}{}%
      \textbf{Predefined node} {\ttfamily\declare{ext\textunderscore#1}}}%
    \index{ext\textunderscore#1@\protect\texttt{ext\textunderscore#1} node}%
    \index{Predefined node!ext\textunderscore#1@\protect\texttt{ext\textunderscore#1}}
    \pgfmanualbody
}
{
  \end{pgfmanualentry}
...
% from: https://github.com/pgf-tikz/pgfplots/blob/3eb58fcae2d14d8fde63e6bab25adaedb0039a1f/doc/latex/pgfplots/pgfplots-macros.tex#L699-L711
\makeatother % FIXME this is almost surely a bug in pgfmanual-en-macros
             % Qrrbrbirlbel: Yes it is. \extractcommand is defined with @ being other
% \begin{commandlist}
%     {\command1{arg1},\command2{\arg2}}
% \end{commandlist}
\renewenvironment{commandlist}[1]{%
    \begin{pgfmanualentry}
    \foreach \xx in {#1} {%
...
\pgfqkeys{/pdflinks}{
  internal link prefix=tikzext,
%  search key prefixes in={/tikz/ext, /pgf/ext, /tikz, /pgf}
}