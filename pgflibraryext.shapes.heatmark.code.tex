\pgfqkeys{/pgf}{
    ...
    heatmark opacity high/.initial=.8,
    heatmark opacity low/.initial=.2,
    heatmark rotate/.initial=90,
    heatmark/.code=\pgfqkeys{/pgf/heatmark}{#1}%
}
...
%
{
  ...
  \behindbackgroundpath{%
    ...
    \pgfmathsetmacro\pgf@tempc{\pgfkeysvalueof{/pgf/heatmark rotate}}%           heat rotate
    %
    \pgfmathsetmacro\pgf@tempd{\pgfkeysvalueof{/pgf/heatmark opacity high}}%     heat opacity high
    \ifnum\c@pgf@counta=1
      \def\qrr@heatmark@opacity@seps{0}%
    \else
      \pgfmathsetmacro\qrr@heatmark@opacity@seps
      {(\pgf@tempd-(\pgfkeysvalueof{/pgf/heatmark opacity low}))%
         /(\c@pgf@counta-1)}%
    \fi
    \pgfutil@tempcnta=0
    \pgfutil@tempcntb=0
    ...
    \pgfutil@loop % ring loop
      ...
      \begingroup
        \edef\pgf@marshal{\noexpand\pgfqkeys{/pgf/heatmark}{ring \number\numexpr\pgfutil@tempcnta+1\relax/.try}}%
        \pgf@marshal
        \tikz@options
        {%
        \pgfutil@loop
          \pgfmathsetmacro\qrr@heatmark@start@angle{\pgf@tempc-.5*\pgf@tempb+\pgfutil@tempcntb*(360/\c@pgf@countb)}%
          % path
          \begingroup
            \edef\pgf@marshal{\noexpand\pgfqkeys{/pgf/heatmark}{arc \the\numexpr\pgfutil@tempcntb+1\relax/.try}%
                              \noexpand\pgfqkeys{/pgf/heatmark}{arc \the\numexpr\pgfutil@tempcnta+1\relax-\the\numexpr\pgfutil@tempcntb+1\relax/.try}}%
            \pgf@marshal
            \csname tikz@options\endcsname
            \pgfpathmoveto{\pgfpointpolar{\qrr@heatmark@start@angle}{\pgf@ya}}%
            \pgfpatharc{\qrr@heatmark@start@angle}{\qrr@heatmark@start@angle+\pgf@tempb}{\pgf@ya}%
            \pgfpathlineto{\pgfpointpolar{\qrr@heatmark@start@angle+\pgf@tempb}{\pgf@yb}}%
            \pgfpatharc{\qrr@heatmark@start@angle+\pgf@tempb}{\qrr@heatmark@start@angle}{\pgf@yb}%
            \pgfpathclose
            \pgfusepathqfill
          \endgroup
      ...
      \ifnum\pgfutil@tempcnta<\c@pgf@counta
        \pgf@ya\pgf@yb
        \pgfmathsetmacro\pgf@tempd{\pgf@tempd-\qrr@heatmark@opacity@seps}%
    \pgfutil@repeat
