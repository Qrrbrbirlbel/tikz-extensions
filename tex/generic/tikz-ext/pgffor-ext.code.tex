...
%%% Compatibility
\tikzextset{
  /tikz-ext/compat/add library={pgffor}{pre 0.6},
  compat/pgffor/warn/.append code={%
    \tikzextutil@compatwarning@key{pgffor}{pre 0.6}{/pgf/foreach/normal list}{/pgf/foreach/ext/normal list}%
    \tikzextutil@compatwarning@key{pgffor}{pre 0.6}{/pgf/foreach/no separator}{/pgf/foreach/ext/no separator}%
    \tikzextutil@compatwarning@key{pgffor}{pre 0.6}{/pgf/foreach/use int}{/pgf/foreach/ext/use int}%
    \tikzextutil@compatwarning@key{pgffor}{pre 0.6}{/pgf/foreach/use float}{/pgf/foreach/ext/use float}%
  },
  compat/pgffor/pre 0.6/.append code={%
  ...
  compat/pgffor/\pgfkeysvalueof{/tikz/ext/compat/pgffor}%
}
\endinput