...
%%
  ...
  pics/softpath arrows/.style={
    ...
    setup code={%
      ...
      \show\pgfprocessresultsubpathprefix
      \show\pgfprocessresultsubpathsuffix
      \expandafter\pgf@parse@end\pgfprocessresultsubpathsuffix\pgf@stop\pgf@stop\pgf@stop
      \pgfutil@switch\pgfutil@ifx\tikz@timer{%
        {\relax}                  {\tikzext@arrow@softpath@curve}%     let's try a curve anyway
        {\tikz@timer@line}        {\tikzext@arrow@softpath@curve}%
        {\tikz@timer@arc}         {\tikzext@arrow@softpath@curve}%     up to four curves
        {\tikz@timer@hvline}      {\tikzext@arrow@softpath@hvline}%
        {\tikz@timer@vhline}      {\tikzext@arrow@softpath@vhline}%
        {\tikz@timer@curve}       {\tikzext@arrow@softpath@curve}%     good
        {\tikzext@timer@rectangle}{\tikzext@arrow@softpath@rectangle}% bad
        {\tikzext@timer@parabola} {\tikzext@arrow@softpath@parabola}%  two curves
        {\tikz@timer@sine}        {\tikzext@arrow@softpath@curve}%     good
        {\tikz@timer@cosine}      {\tikzext@arrow@softpath@curve}%     good
...
% -| and |-
\def\tikzext@arrow@hvpath{%
  \pgfmathparse{\pgfkeysvalueof{/tikz/pos <}}%
  \pgf@xa=\pgfmathresult pt
  \pgf@xa=2\pgf@xa
  \edef\tikz@time@{\the\pgf@xa}%
  \ifdim\pgf@xa>1pt % < on second leg
    \pgfpathmoveto{%
      \pgfpointlineattime
        {\tikz@time@-1}
        {\tikzext@secondlastonpath}
        {\tikzext@lastonpath}%
    }%
    \pgfpathlineto{% > better be on the second leg as well
      \pgfpointlineattime
        {2*\tikz@time-1}
        {\tikzext@secondlastonpath}
        {\tikzext@lastonpath}%
  ...
  \else % < on first leg
    ...
    \ifdim\tikz@time pt>.5pt % > on second leg
      \pgfpathlineto{\tikzext@secondlastonpath}%
      \pgfpathlineto{%
        \pgfpointlineattime
          {2*\tikz@time-1}
          {\tikzext@secondlastonpath}
