...
\pgfkeysdef{/tikz/auto/false}{\let\tikz@do@auto@anchor\relax}
\pgfkeysdefnargs{/tikz/ext/nodes/install auto}{2}{%
  \pgfkeysdef{/tikz/swap/#1}{\def\tikz@auto@anchor@direction{#2\iftikzext@auto@precise @precise\fi}}%
  \pgfkeysdef{/tikz/auto/#1}{\let\tikz@do@auto@anchor\tikz@auto@anchor@on\def\tikz@auto@anchor@direction{#1\iftikzext@auto@precise @precise\fi}}%
  \pgfkeysdef{/tikz/swap/#1@precise}{\def\tikz@auto@anchor@direction{#2\iftikzext@auto@precise @precise\fi}}%
  \pgfkeysdef{/tikz/auto/#1@precise}{\let\tikz@do@auto@anchor\tikz@auto@anchor@on\def\tikz@auto@anchor@direction{#1\iftikzext@auto@precise @precise\fi}}%
}
\tikzset{
...
\def\tikzext@auto@pre{%
  ...
  \tikz@auto@pre % includes \begingroup
  \endgroup
}
\def\tikzext@auto@post#1{%
  \iftikzext@auto@offset
    \pgfmathsetmacro\tikzext@auto@offset@value{\pgfkeysvalueof{/tikz/ext/auto offset}}%
    \ifpgfslopedattime
      \tikz@addtransform{\csname tikzext@auto@offset@\tikz@anchor\endcsname}%
    \else
      \tikz@addtransform{\csname tikzext@auto@offset@#1\endcsname}%
    \fi
  \fi
}
\pgfutil@namedef{tikzext@auto@offset@above}{\pgftransformshift{\pgfqpointscale{\tikzext@auto@offset@value}{\pgfqpoint{+\tikzext@unslopedx}{+\tikzext@unslopedy}}}}
\pgfutil@namedef{tikzext@auto@offset@left}{\pgftransformshift{\pgfqpointscale{\tikzext@auto@offset@value}{\pgfqpoint{-\tikzext@unslopedy}{+\tikzext@unslopedx}}}}
\pgfutil@namedef{tikzext@auto@offset@below}{\pgftransformshift{\pgfqpointscale{\tikzext@auto@offset@value}{\pgfqpoint{-\tikzext@unslopedx}{-\tikzext@unslopedy}}}}
\pgfutil@namedef{tikzext@auto@offset@right}{\pgftransformshift{\pgfqpointscale{\tikzext@auto@offset@value}{\pgfqpoint{+\tikzext@unslopedy}{-\tikzext@unslopedx}}}}
\pgfutil@namedef{tikzext@auto@offset@north}{\pgftransformyshift{+-\tikzext@auto@offset@value pt}}%
\pgfutil@namedef{tikzext@auto@offset@south}{\pgftransformyshift{++\tikzext@auto@offset@value pt}}%
\pgfutil@namedef{tikzext@auto@offset@east}{\pgftransformxshift{+-\tikzext@auto@offset@value pt}}%
\pgfutil@namedef{tikzext@auto@offset@west}{\pgftransformxshift{++\tikzext@auto@offset@value pt}}%

% direction: Left
\def\tikzext@auto@leftright#1#2#3#4{%
  \tikzext@auto@pre
...
\pgfutil@namedef{tikz@auto@anchor@ext/left}{\tikzext@auto@leftright{left}{south}{north}{\tikz@auto@anchor}}
\pgfutil@namedef{tikz@auto@anchor@ext/right}{\tikzext@auto@leftright{right}{north}{south}{\tikz@auto@anchor@prime}}

\def\tikzext@auto@northsouth#1#2#3#4#5#6{%
  \tikzext@auto@pre
  \def\tikz@anchor{#3}%
  \def\tikzext@autooffset@angle{#1}%
  \iftikzext@slopedattime
    \iftikzext@allowupsidedownattime
      \ifdim\tikzext@unslopedx<0pt
        \def\tikz@anchor{#4}%
      \fi
      % we don't need an \else case here
      % because the anchor will always be
      % south for ext/north and north for ext/south
    \fi
  \else
    \ifdim\pgf@x>0.05pt
      \ifdim\pgf@y>0.05pt
    ...
    \fi\fi
  \fi
  \tikzext@auto@post{\tikzext@autooffset@angle}%
}
\pgfutil@namedef{tikz@auto@anchor@ext/north}{\tikzext@auto@northsouth{left}{right}{south}{north}{east}{west}}
\pgfutil@namedef{tikz@auto@anchor@ext/south}{\tikzext@auto@northsouth{right}{left}{north}{south}{west}{east}}

\def\tikzext@auto@eastwest#1#2#3#4#5#6{%
  \tikzext@auto@pre
  \def\tikzext@autooffset@angle{#1}%
  \iftikzext@slopedattime
    \def\tikz@anchor{#5}%
...
\pgfutil@namedef{tikz@auto@anchor@ext/east}{\tikzext@auto@eastwest{right}{left}{west}{east}{north}{south}}
\pgfutil@namedef{tikz@auto@anchor@ext/west}{\tikzext@auto@eastwest{left}{right}{east}{west}{south}{north}}

% auto: above/below (always towards end/start of line)
\def\tikzext@auto@abovebelow#1#2#3#4#5{%
  \tikzext@auto@pre
  \iftikzext@slopedattime
    \def\tikz@anchor{#4}%
    \iftikzext@allowupsidedownattime
...
\pgfutil@namedef{tikz@auto@anchor@ext/below}{\tikzext@auto@abovebelow{below}{north}{south}{east}{west}}
\pgfutil@namedef{tikz@auto@anchor@ext/above}{\tikzext@auto@abovebelow{above}{south}{north}{west}{east}}

%%% auto: precise %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% no auto offset and no precise for original left and right options
\let\tikz@auto@anchor@left@precise\tikz@auto@anchor@left
\let\tikz@auto@anchor@right@precise\tikz@auto@anchor@right

\def\tikzext@auto@pre@precise{%
  \pgfmathsetmacro\tikzext@auto@offset@value{\pgfkeysvalueof{/tikz/ext/auto offset}}%
  \tikz@auto@pre % includes a \begingroup
  \endgroup
  \edef\tikzext@unslopedx{\the\pgf@x}%
  \edef\tikzext@unslopedy{\the\pgf@y}%
  \pgfkeys@expanded{%
...
\def\tikzext@auto@post@precise#1{%
  \let\tikz@anchor\pgfmathresult
  \iftikzext@auto@offset
    \ifpgfslopedattime
      % this should never happen, sloped disables precise
    \else
      \tikz@addtransform{\csname tikzext@auto@offset@#1\endcsname}%
    \fi
  \fi
...
\pgfutil@namedef{tikz@auto@anchor@ext/left@precise}{%
  \tikzext@auto@pre@precise
  \pgfmathadd@{\pgfmathresult}{-90}%
  \tikzext@auto@post@precise{90}
}
...
\pgfutil@namedef{tikz@auto@anchor@ext/north@precise}{%
  ...
  \else
    ...
    \def\tikzext@autooffset@angle{90}%
  \fi
  \tikzext@auto@post@precise{\tikzext@autooffset@angle}%
}
...
\pgfutil@namedef{tikz@auto@anchor@ext/above@precise}{%
  ...
  \tikzext@auto@post@precise{0}%
}
\pgfutil@namedef{tikz@auto@anchor@ext/below@precise}{%
  \tikzext@auto@pre@precise
  \let\tikz@anchor\pgfmathresult
  \tikzext@auto@post@precise{180}%
}
%%% node pic
...
\let\tikzext@nodepicture@hooklist\pgfutil@gobble
\newif\iftikzext@nodepicture@ilw
\newif\iftikzext@nodepicture@prefix
\newif\iftikzext@nodepicture@nonode
...
\tikzextset{
  ...
  node picture/no node/.is if=tikzext@nodepicture@nonode
}
...
\def\tikzext@nodepicture@callback@#1{%
  ...
  \begingroup
    ...
    \pgftransformshift{\pgfqpoint{-\tikzext@nodepicture@minx}{-\tikzext@nodepicture@baseline}}%
...
\def\tikzext@nodepicture@hook{%
  \xdef\tikzext@nodepicture@hooklist{\tikzext@nodepicture@hooklist,{\the\tikzext@nodepicture}{\the\pgf@size@hook@x}{\the\pgf@size@hook@y}}%
}
\def\tikzext@nodepicture@callback@size#1{%
  \unless\ifx\tikzext@nodepicture@hooklist@this\pgfutil@gobble
    \begingroup
      % First install the transformation matrix of the node picture's node.
      \pgfsettransform{\csname pgf@sh@nt@#1\endcsname}%
      \csname pgf@sh@np@#1\endcsname
      ...
      % Correction: Baseline and left side of node picture.
      \pgftransformshift{\pgfqpoint{-\tikzext@nodepicture@minx}{-\tikzext@nodepicture@baseline}}%
      \pgfutil@for\tikzext@temp:=\tikzext@nodepicture@hooklist@this\do{%
        \expandafter\tikzext@nodepicture@callback@hook@special\tikzext@temp
      }%
    \endgroup
  \fi
}
\def\tikzext@nodepicture@callback@hook@special#1#2#3{%
  \pgf@process{\pgfpointtransformed{\pgf@x=#2 \pgf@y=#3}}%
  \pgf@protocolsizes\pgf@x\pgf@y
}
% node picture: baseline
\def\tikzext@nodepicture@baseline@coord#1{%
  \pgf@process{\pgfpointtransformed{#1}}%
  \edef\tikzext@nodepicture@baseline{\the\pgf@y}%
}
\def\tikzext@nodepicture@baseline@value#1{%
  \pgfmathsetlength\pgf@ya{#1}%
  \edef\tikzext@nodepicture@baseline{\the\pgf@ya}%
}
\def\tikzext@nodepicture@shift#1{%
  \pgf@process{\pgfpointtransformed{#1}}%
  \edef\tikzext@nodepicture@shiftvalue{\noexpand\pgfqpoint{\the\pgf@x}{\the\pgf@y}}%
}
...
%%% Now the real Node Picture stuff
\let\tikzextorig@tikz@do@fig\tikz@do@fig
\def\tikzext@nodepicture@do{%
  \tikzset{ext/every node picture=\the\tikzext@nodepicture}%
  \tikzextutil@prefixtomacro\tikz@alias{%
    \tikzext@nodepicture@callback@{\tikz@fig@name}%
    \tikzext@nodepicture@callback@anchors{\tikz@fig@name}%
    \iftikzext@nodepicture@nonode
      \tikzext@nodepicture@callback@size{\tikz@fig@name}%
    \fi
  }%
  ...
  \let\tikzext@nodepicture@hooklist@save\tikzext@nodepicture@hooklist
  \global\let\tikzext@nodepicture@hooklist\pgfutil@gobble
    ...
    \pgfinterruptpath
      \pgfscope
        \pgfinterruptboundingbox
          ...
          \iftikzext@nodepicture@nonode
            \expandafter\def\expandafter\pgf@path@size@hook\expandafter{\pgf@path@size@hook\tikzext@nodepicture@hook}
          \fi
...
\def\tikzext@nodepicture@done{%
            ...
            \ifx\tikzext@nodepicture@anchor@save\pgfutil@empty
              \let\tikzext@nodepicture@shiftvalue\pgfutil@empty
            \else
              \def\tikz@anchor{text}%
            ...
            \fi
            \xdef\tikzext@gtemp{%
              \pgfutil@unexpanded{\def\noexpand\tikzext@nodepicture@baseline}{\tikzext@nodepicture@baseline}%
              \pgfutil@unexpanded{\def\noexpand\tikzext@nodepicture@shiftvalue}{\tikzext@nodepicture@shiftvalue}%
            }%
          \endscope
  ...
  %
  \ifx\tikzext@nodepicture@shiftvalue\pgfutil@empty
  \else
    \def\tikz@anchor{text}%
    \tikz@addtransform{%
      \pgftransformshift{%
        \tikzext@nodepicture@shiftvalue
        \pgf@x=-\pgf@x
  ...
  \fi
  \let\tikzext@nodepicture@hooklist@this\tikzext@nodepicture@hooklist
  \global\let\tikzext@nodepicture@hooklist\tikzext@nodepicture@hooklist@save
  \iftikzext@nodepicture@nonode
    \pgfutil@namelet{pgf@sh@bbg@\tikz@shape}{pgfutil@empty}%
    \pgfutil@namelet{pgf@sh@bg@\tikz@shape}{pgfutil@empty}%
    \pgfutil@namelet{pgf@sh@fbg@\tikz@shape}{pgfutil@empty}%
    \pgfutil@namelet{pgf@sh@fbg@\tikz@shape}{pgfutil@empty}%
    \pgfutil@namelet{pgf@sh@fg@\tikz@shape}{pgfutil@empty}%
    \pgfutil@namelet{pgf@sh@ffg@\tikz@shape}{pgfutil@empty}%
  \fi
  \tikzset{ext/every outer node picture=\the\tikzext@nodepicture}%
  \tikz@fig@continue
}
