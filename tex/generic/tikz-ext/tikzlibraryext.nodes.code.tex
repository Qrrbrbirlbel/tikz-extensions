...
\newcount\tikzext@picturenode
\let\tikzext@picturenode@list\pgfutil@gobble

\def\tikzext@picturenode@callback#1{%
  \def\tikzext@temp{#1}%
  \unless\ifx\pgfutil@empty\tikzext@temp
    \xdef\tikzext@picturenode@list{\tikzext@picturenode@list,{\the\tikzext@picturenode}{#1}}%
  \fi
}
...
\def\tikzext@picturenode@callback@#1{%
  ...
  \begingroup
    ...
    \pgftransformshift{\pgfqpoint{-\csname pgf@lbb@minx@tikzext@picturenode\the\tikzext@picturenode\endcsname}{0pt}}
    ...
    \pgftransformyshift{+-\tikzext@nodepicture@baseline@}%
    \let\tikzext@temp\tikzext@picturenode@list
    \global\let\tikzext@picturenode@list\pgfutil@gobble
    \pgfutil@for\tikzext@temp:=\tikzext@temp\do{%
      \expandafter\tikzext@picturenode@callback@special\tikzext@temp
    }%
  \endgroup
  \fi
}
\def\tikzext@picturenode@callback@special#1#2{%
  \ifnum\tikzext@picturenode>\pgfinteval{#1-1}
    \xdef\tikzext@picturenode@list{\tikzext@picturenode@list,{#1}{#2}}%
  \else
    \unless\ifcsname tikzext@picturenode@#2-#1@visited\endcsname
      \expandafter\let\csname tikzext@picturenode@#2-#1@visited\endcsname\pgfutil@empty
      \begingroup
      ...
      \endgroup
      \ifnum#1>1
        \xdef\tikzext@picturenode@list{\tikzext@picturenode@list,{\pgfinteval{#1-1}}{#2}}%
      \fi
    \fi
  \fi
}
...
\def\tikzext@picturenode@do{%
  \pgfkeysvalueof{/tikz/prefix after command/.@cmd}%
    \pgfextra{%
      \tikzext@picturenode@callback@{\tikzlastnode}%
    }%
  \pgfeov
    ...
    \pgfinterruptpath
      \pgfscope
        \pgfinterruptboundingbox
          \pgftransformreset
          \edef\tikzext@temp{%
            \noexpand\pgfkeysvalueof{/pgf/local bounding box/.@cmd}tikzext@picturenode\the\tikzext@picturenode\noexpand\pgfeov
          }\tikzext@temp
          \advance\tikzext@picturenode by1
          ...
          \bgroup
            \expandafter\def\csname pgf@sh@np@current bounding box\endcsname{%
              \def\southwest{\pgfqpoint{\csname pgf@lbb@minx@tikzext@picturenode\pgfinteval{\tikzext@picturenode-1}\endcsname}%
                                       {\csname pgf@lbb@miny@tikzext@picturenode\pgfinteval{\tikzext@picturenode-1}\endcsname}}%
              \def\northeast{\pgfqpoint{\csname pgf@lbb@maxx@tikzext@picturenode\pgfinteval{\tikzext@picturenode-1}\endcsname}%
                                       {\csname pgf@lbb@maxy@tikzext@picturenode\pgfinteval{\tikzext@picturenode-1}\endcsname}}%
            }%
            \edef\tikzextnodepicture{\pgfinteval{\tikzext@picturenode-1}}%
            \aftergroup\tikzext@picturenode@done
}
\def\tikzext@picturenode@done{
          \ifx\tikzext@picturenode@baseline\pgfutil@empty
            \gdef\tikzext@nodepicture@baseline@{0pt}%
          \else
            \def\tikzext@temp{\tikz@handle@vec{\tikzext@picturenode@baseline@coord}{\tikzext@picturenode@baseline@value}}%
            \expandafter\tikzext@temp\tikzext@picturenode@baseline\relax
          \fi
  ...
  \setbox\pgfnodeparttextbox=\hbox{%
    \kern-\csname pgf@lbb@minx@tikzext@picturenode\the\tikzext@picturenode\endcsname
    \lower\tikzext@nodepicture@baseline@\relax
    \box\pgfnodeparttextbox
    \kern\csname pgf@lbb@maxx@tikzext@picturenode\the\tikzext@picturenode\endcsname
  }%
  \pgf@ya=\csname pgf@lbb@maxy@tikzext@picturenode\the\tikzext@picturenode\endcsname\relax
  \advance\pgf@ya by -\tikzext@nodepicture@baseline@
  \ht\pgfnodeparttextbox=+\pgf@ya
  \pgf@ya=-\csname pgf@lbb@miny@tikzext@picturenode\the\tikzext@picturenode\endcsname\relax
  \advance\pgf@ya by \tikzext@nodepicture@baseline@
  \dp\pgfnodeparttextbox=\pgf@ya
  \tikz@fig@continue
}
\def\tikzext@picturenode@baseline@coord#1{%
  \xdef\tikzext@nodepicture@baseline@{\the\pgf@y}%
}
\def\tikzext@picturenode@baseline@value#1{%
...
\pgfkeysdef{/tikz/ext/baseline}{\gdef\tikzext@picturenode@baseline{#1}}
\global\let\tikzext@picturenode@baseline\pgfutil@empty
