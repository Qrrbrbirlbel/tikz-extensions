...
\newdimen\tikzext@ortho@distance
\newif\iftikzext@ortho@middle         % checks whether spacing = 0 was selected
...
\tikzextset{%
  ...
  %                                           the first part is -1 to 0, the last part is to 2
    ...
    \fi,%
  ortho/ratio/.code=\pgfmathsetmacro\tikzext@ortho@ratio{#1},%
  % TODO: not possible to do distance in TikZ coordinate system
  ortho/distance/.code=\pgfmathsetlength\tikzext@ortho@distance{#1}\let\tikzext@ortho@ratio\pgfutil@empty,%
  ...
  \begingroup
    ...
    \iftikz@shapeborder
      ...
      \ifx\tikzext@ortho@ratio\pgfutil@empty
        \ifdim\tikzext@ortho@distance<0pt\relax
          \iftikzext@ortho@fromcenter
            \pgf@xc\tikz@lastx
          \else
            \pgf@xc\pgf@x
        ...
        \ifdim\pgf@xb>\tikz@lastx
          \tikzext@ortho@distance-\tikzext@ortho@distance
        \fi
      \else
        \iftikzext@ortho@fromcenter
          \tikzext@ortho@distance\tikz@lastx
        \else
          \tikzext@ortho@distance\pgf@x
        \fi
        \advance\tikzext@ortho@distance-\pgf@xc
        \tikzext@ortho@distance\tikzext@ortho@ratio\tikzext@ortho@distance
      \fi
      \advance\pgf@xc\tikzext@ortho@distance
...
\def\tikzext@ortho@vhv@do#1{%
  ...
  \begingroup
    ...
    \iftikz@shapeborder
      ...
      \else
        \iftikzext@ortho@fromcenter
          \tikzext@ortho@distance\tikz@lasty
        \else
          \tikzext@ortho@distance\pgf@y
        \fi
        \advance\tikzext@ortho@distance-\pgf@yc
        \tikzext@ortho@distance\tikzext@ortho@ratio\tikzext@ortho@distance
      \fi
      \advance\pgf@yc\tikzext@ortho@distance
      \edef\pgf@tempa{\noexpand\pgfqpoint{\the\pgf@xc}{\the\pgf@yc}}%
...
\def\tikzext@ortho@uddu@do#1#2#3#4{%
 \edef\tikz@timer@start{\noexpand\pgfqpoint{\the\tikz@lastx}{\the\tikz@lasty}}%
  ...
  \begingroup
    \def\tikzext@temp{\pgfqkeys{/tikz/ext/ortho}}%
    \expandafter\tikzext@temp\expandafter{\tikzext@ortho@options}%
    \pgfmathsetlength\pgfutil@tempdima{\pgfkeysvalueof{/tikz/ext/ortho/#3 distance}}%
    \iftikz@shapeborder
      \advance\pgf@y#21pt%
