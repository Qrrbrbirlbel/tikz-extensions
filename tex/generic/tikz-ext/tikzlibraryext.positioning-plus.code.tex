...
\def\tikzext@posp@of@firsttwo#1#2\pgf@stop{%
  \def\tikzext@tempa{#1}%
  \def\tikzext@tempb{#2}%
  \ifx\tikzext@tempb\tikzext@@stop
    \let\tikzext@tempb\pgfutil@empty
  \else
    \tikzext@posp@of@firsttwo@#2\pgf@stop
  \fi
}
\def\tikzext@posp@of@firsttwo@#1#2\pgf@stop{%
  \def\tikzext@tempb{#1}%
}
\def\tikzext@@stop{\tikzext@@stop}%
\def\tikzext@posp@of@#1{%
  \expandafter\tikzext@posp@of@@\pgfkeys@expanded{{#1}}%
}
\def\tikzext@posp@of@@#1#2{%
  % #1: of part
  % #2: other anchor
  \def\tikzext@posp@scan{ext_fit bounding box}%
  \tikzext@posp@of@firsttwo#1\tikzext@@stop\pgf@stop
  \ifx\tikzext@tempb\pgfutil@empty
    \def\tikzext@posp@scan{#1}%
  \else
    \if\tikzext@tempb(% alright, we need to fit something
      \if\tikzext@tempa|%
          \expandafter\tikzext@posp@fit\expandafter{\tikzextutil@secondofmany#1\pgf@stop}{1}%
        \else\ifx\tikzext@tempa\tikz@activebar
            \expandafter\tikzext@posp@fit\expandafter{\tikzextutil@secondofmany#1\pgf@stop}{1}%
          \else\if\tikzext@tempa-%
            ...
            \else\if\tikzext@tempa+%
              ...
              \else
                \pgfutil@packagewarning{tikz-ext}{%
                  Unknown fit type '\tikzext@tempa' for '\tikzextutil@secondofmany#1\pgf@stop'.%
                }%
                \expandafter\tikzext@posp@fit\expandafter{\tikzextutil@secondofmany#1\pgf@stop}{0}%
            \fi
          \fi
        \fi
      \fi
    \else % no fit but still maybe?
      \if\tikzext@tempa(%
        \tikzext@posp@fit{#1}{0}%
      \else % normal of
        \def\tikzext@posp@scan{#1}%
...
\tikzextset{
  fit bounding box/.code=\tikzext@posp@fit{#1}{0},
  span vertical/.code=\tikzext@posp@fit{#1}{1},
  span horizontal/.code=\tikzext@posp@fit{#1}{2},
  span/.code=\tikzext@posp@fit{#1}{3}}
